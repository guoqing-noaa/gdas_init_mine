...
if [ $RUN_CHGRES == yes ]; then
  ...
  case $gfs_ver in
    ...
    v16retro)
      if [ "$CDUMP" = "gdas" ] ; then
        sbatch --parsable --ntasks-per-node=6 --nodes=${NODES} -t $WALLT -A $PROJECT_CODE -q $QUEUE -J chgres_${CDUMP} \
        -o log.${CDUMP} -e log.${CDUMP} --partition=${PARTITION} ${DEPEND} run_v16retro.chgres.sh hires
      else
        sbatch --parsable --ntasks-per-node=6 --nodes=${NODES} -t $WALLT -A $PROJECT_CODE -q $QUEUE -J chgres_${CDUMP} \
  ...
  if [ "$CDUMP" = "gdas" ]; then
    ...
    if [ "$gfs_ver" = "v16retro" ]; then
      ...
      -o log.enkf -e log.enkf --partition=${PARTITION} ${DEPEND} run_v16retro.chgres.sh enkf
